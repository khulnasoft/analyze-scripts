...
def generate_report(
    ...
    report_template_data = {'sub_analyses': sub_analyses,
                            ...
                            'analyze_base_url': 'https://analyze.khulnasoft.com'}
...
def generate_reports(khulnasoft_api_key, endpoint_analyses_ids):
    api.set_global_api(khulnasoft_api_key)
    ...
    with open('khulnasoft-logo.png', 'rb') as image_file:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate a PDF and HTML reports for a given Khulnasoft endpoint scans')
    parser.add_argument('-k', '--api-key', help='Khulnasoft API Key', required=True)
